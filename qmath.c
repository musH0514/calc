...
NUMBER *initnumbs[] = {
    &_qzero_, &_qone_, &_qtwo_, &_qten_, &_qnegone_, &_qonehalf_, &_qneghalf_, &_qonesqbase_, NULL /* must be last */
};
...
qfrac(NUMBER *q)
{
    ...
    if ((q->num.len < q->den.len) || ((q->num.len == q->den.len) && (q->num.v[q->num.len - 1] < q->den.v[q->num.len - 1]))) {
        return qlink(q);
    }
    r = qalloc();
